        ...
        itemBuilder: (context, index) {
          if (index < messages.length) {
            ...
            if (message.user?.id == context.currentUser?.id) {
              return _OurMessages(message: message);
            } else {
...
class _CustomAppBarTitle extends StatelessWidget {
  const _CustomAppBarTitle({Key? key}) : super(key: key);
...
class _DateLableState extends State<_DateLable> {
  ...
  void initState() {
    ...
    if (Jiffy(createdAt).isSame(now, Units.DAY)) {
      ...
      now.subtract(const Duration(days: 7)), //more than a week
      Units.DAY,
    )) {
      dayInfo = createdAt.EEEE;
    } else if (Jiffy(createdAt).isAfter(
      Jiffy(now).subtract(years: 1), //more than year
      Units.DAY,
    )) {
          ...
          children: [
            ...
            Container(
              ...
              child: Padding(
                ...
                child: Text(message.text ?? 'Message null',
                    style: const TextStyle(
...
// wiget for typeing messages and send.
          ...
          Expanded(
            child: Padding(
              ...
              child: TextField(
                ...
                onChanged: (val) => StreamChannel.of(context)
                    .channel
                    .keyStroke(), //when user typing, the status will cahnge to typeing.
